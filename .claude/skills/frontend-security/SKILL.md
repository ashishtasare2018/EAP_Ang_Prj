...
# Angular sanitizer bypasses and unsafe bindings
...
# Angular HttpClient without XSRF configuration
grep -rn "HttpClientModule\|provideHttpClient" --include="*.ts"
grep -rn "HttpClientXsrfModule\|withXsrfConfiguration" --include="*.ts"
```
...
- **Framework-specific patterns**: See `references/framework-patterns.md` (includes an Angular section: sanitizer bypasses, `[innerHTML]`, XSRF config, route guards, reactive form validation)
